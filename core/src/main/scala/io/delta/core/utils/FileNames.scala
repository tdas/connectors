...
object FileNames {
  ...
  def deltaFile(path: String, version: Long): String = concatenatePath(path, f"$version%020d.json")

  /** Returns the version for the given delta path. */
  def deltaVersion(path: String): Long = getFileName(path).stripSuffix(".json").toLong
   ...
   */
  def checkpointPrefix(path: String, version: Long): String =
    concatenatePath(path, f"$version%020d.checkpoint")
   ...
   */
  def checkpointFileSingular(path: String, version: Long): String =
    concatenatePath(path, f"$version%020d.checkpoint.parquet")
   ...
   */
  def checkpointFileWithParts(path: String, version: Long, numParts: Int): Seq[String] = {
    val pathAsJavaPath = new File(path).toPath
    Range(1, numParts + 1)
      .map(i =>
        pathAsJavaPath
          .resolve(f"$version%020d.checkpoint.$i%010d.$numParts%010d.parquet")
          .toString
      )
  }

  def numCheckpointParts(path: String): Option[Int] = {
    val segments = getFileName(path).split("\\.")

    if (segments.size != 5) None else Some(segments(3).toInt)
  }

  def isCheckpointFile(path: String): Boolean =
    checkpointFilePattern.matcher(getFileName(path)).matches()

  def isDeltaFile(path: String): Boolean = deltaFilePattern.matcher(getFileName(path)).matches()

  def checkpointVersion(path: String): Long = getFileName(path).split("\\.")(0).toLong
   ...
   * upgrade.
   */
  def getFileVersion(path: String): Long = {
   ...
   */
  def absolutePath(parentDir: String, child: String): String = {
    new File(parentDir).toPath.resolve(child).toString
  }

  def concatenatePath(parent: String, child: String): String = {
    new File(parent).toPath.resolve(child).toString
  }
